...
class PortalDashboardTests(unittest.TestCase):
    def test_consolidation_preview_and_large_metrics(self):
        from consolidation import ConsolidationRun
        from test_consolidation import Fake,source,ORG,APP
        with tempfile.TemporaryDirectory() as directory:
            Fake.rows=[source(i) for i in range(1,11)]+[dict(source(99),notes='Unrelated')]
            cfg=Settings(ORG,'fake-token');run=ConsolidationRun(cfg,directory,Fake);run.prepare(APP,'balanced')
            app=AppTest.from_file('portal_app.py').run(timeout=20)
            app.text_input(key='org').set_value(ORG);app.text_input(key='token').set_value('fake-token')
            app.session_state['consolidation_run']=run
            app.session_state['large_scope']=(ORG,'d','D')
            app.session_state['large_rows']=[{'Application':'Large test','Application ID':APP,'Reported hashes':1001}]
            app.run(timeout=20)
            self.assertEqual(len(app.exception),0)
            metrics={m.label:m.value for m in app.metric}
            self.assertEqual(metrics['Covered hash records'],'10')
            self.assertEqual(metrics['Proposed conditional rules'],'1')
            self.assertEqual(metrics['Reported hashes'],'1,001')
            self.assertTrue(next(b for b in app.button if b.label=='Apply reviewed conditional rules').disabled)
            self.assertGreaterEqual(len(app.get('progress')),2)
