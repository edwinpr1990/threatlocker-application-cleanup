...
from portal_theme import apply_theme,header,application_chart,CYAN,GREEN
from guided_ui import picker,stepper,comparison,activity,overview,history_rows

st.set_page_config(page_title='Application Hash Health · RC2',page_icon='🛡️',layout='wide')
apply_theme()
...
with st.sidebar:
    ...
    if st.button('Disconnect / clear credentials'):
        active_runs=[r for r in (st.session_state.get('run'),st.session_state.get('consolidation_run')) if r and r.active]
        if active_runs:
            for active in active_runs:active.stop.set()
            st.warning('Stopping. Disconnect after the current requests finish.')
        else:
...
activity(cfg)
overview_tab,report_tab,execution_tab,history_tab,conditions_tab,large_tab=st.tabs(['Overview','Report and preview','Execution and audit','Run history','Application cleanup','Large applications'],key='workspace_tab',on_change='rerun')
with report_tab:
    stepper(0)
    has_report=bool(cfg and st.session_state.get('candidate_org')==cfg.org and st.session_state.get('candidates'))
    with st.expander('Load or refresh report',expanded=not has_report):
    ...
    with st.expander(f'Application and OS filters · {len(choices)} applications available',expanded=not choices):
    ...
    with st.expander('Scan additional applications, including macOS'):
        ...
        picked_apps=picker(cfg,'hash_picker',multiple=True)
    ...
    if st.button('Build dry-run preview',type='primary',disabled=cfg is None or bool(active and active.active)):
        try:
            apps=list(dict.fromkeys(picked_apps+[uid(a.strip()) for a in extra.splitlines() if a.strip()]))
    ...
    if scoped:
        ...
        with st.container():
            ...
            application_chart(ranked,[('Estimated extra records',CYAN)])
            st.caption('Top 15 applications by known estimated extra records. Full IDs are listed below.')
        with st.expander('How the preview protects your application rules'):
...
with execution_tab:
    ...
    def monitor():
        ...
        stepper(3 if run.active and audit['metadata'].get('prepared') else 1 if run.active else 4 if audit['metadata'].get('phase')=='Complete' else 2)
        ...
        if apps:
            comparison(sum(r['Baseline records'] for r in apps),counts['planned'],complete=audit['metadata'].get('phase')=='Complete')
            left,right=st.columns([3,2])
            with left:
                st.markdown('**Cleanup by application**')
                application_chart(apps,[('Verified deletions',GREEN),('Remaining planned',CYAN)])
            with right:
...
with history_tab:
    st.subheader('Run history and recovery')
    st.caption('Both cleanup workflows, scoped to the current connection. Loading a run never starts execution.')
    rows=history_rows(cfg,RUNS)
    workflow=st.selectbox('History workflow',['All','Duplicate hashes','Conditional rules'])
    history_search=st.text_input('Find a past run',placeholder='Application, ID or run name')
    rows=[r for r in rows if (workflow=='All' or r['Workflow']==workflow) and history_search.lower() in (r['Run']+' '+r['Application']+' '+r['Application IDs']).lower()]
    if rows:
        table=[{k:(datetime.fromtimestamp(v).strftime('%Y-%m-%d %H:%M') if k=='Created' else v) for k,v in r.items() if k!='directory'} for r in rows]
        st.dataframe(table,width='stretch',hide_index=True)
        st.download_button('Download run history CSV',pd.DataFrame(table).to_csv(index=False).encode(),'run-history.csv','text/csv')
    else:st.info('No saved runs match this connection and filter.')
    lookup={r['directory']:r for r in rows}
    selected_run=st.selectbox('Run directory',list(lookup),format_func=lambda p:lookup[p]['Workflow']+' · '+lookup[p]['Run'],index=None)
    running=any(r and r.active for r in (st.session_state.get('run'),st.session_state.get('consolidation_run')))
    if st.button('Load saved run',disabled=selected_run is None or running):
        row=lookup[selected_run]
        if row['Workflow']=='Conditional rules':
            from consolidation import ConsolidationRun
            st.session_state['consolidation_run']=ConsolidationRun(cfg,selected_run)
            st.session_state['conditions_reviewed']=False;st.session_state['conditions_confirm']=''
            st.success('Loaded. Open Application cleanup to inspect the saved result or resume.')
        else:
            st.session_state['run']=Run(cfg,selected_run)
            st.session_state['run'].update(phase='Loaded; reconcile before resuming')
            st.success('Loaded. Open Execution and audit to inspect the saved result or resume.')

with conditions_tab:
    from consolidation_ui import render as render_consolidation
    render_consolidation(cfg,ROOT)
with large_tab:
    from large_apps import render as render_large_apps
    render_large_apps(cfg)
